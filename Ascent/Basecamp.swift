...
struct BasecampView: View {
                        ...
                        ZStack {
                            ...
                            if let urlString = appState.avatarURL, let url = URL(string: urlString) {
                                ...
                                Circle().fill(Color.white).frame(width: 38, height: 38)
                                    .overlay(Image(systemName: "person.fill").font(DesignSystem.Typography.appFont(size: 16)).foregroundColor(tierColor))
                            }
                        }
                    }
    ...
    private func sectionHeader(_ title: String, icon: String, iconColor: Color) -> some View {
        HStack(spacing: 8) {
            Image(systemName: icon).font(DesignSystem.Typography.appFont(size: 16, weight: .bold)).foregroundColor(iconColor)
            Text(title.uppercased()).font(DesignSystem.Typography.appFont(size: 14, weight: .black)).foregroundColor(.secondary).tracking(1)
            Spacer()
        }
        .padding(.horizontal, 16)
    }
    ...
    private func peakPlaceholder(_ peak: Mountain) -> some View {
        ZStack {
            ...
            Image(systemName: peak.isPrestigePeak ? "crown.fill" : "mountain.2.fill")
                .font(DesignSystem.Typography.appFont(size: 50)).foregroundColor(.white.opacity(0.08))
        }
    }
}
...
struct RouteCard: View {
                    ...
                    if let credit = mountain.image_credit, !credit.isEmpty {
                        VStack {
                            ...
                            HStack {
                                Spacer()
                                Text("Foto: \(credit)")
                                    .font(DesignSystem.Typography.appFont(size: 6, weight: .medium))
                                    .foregroundColor(.white.opacity(0.4))
                                    .padding(.trailing, 6)
                                    .padding(.bottom, 4)
                            }
    ...
    private var routePlaceholder: some View {
        ZStack {
            ...
            Image(systemName: "mountain.2.fill").font(DesignSystem.Typography.appFont(size: 28)).foregroundColor(.white.opacity(0.12))
        }
    }
}
            ...
            HStack(spacing: 12) {
                ZStack {
                    ...
                    Image(systemName: mountain.isPrestigePeak ? "crown.fill" : "mountain.2.fill")
                        .font(DesignSystem.Typography.appFont(size: 18)).foregroundColor(mountain.isPrestigePeak ? gold : .blue)
                }
                VStack(alignment: .leading, spacing: 3) {
                    Text(mountain.name).font(DesignSystem.Typography.appFont(size: 14, weight: .bold)).foregroundColor(.primary).lineLimit(1)
                    Text("\(mountain.elevation)m · \(mountain.region)").font(DesignSystem.Typography.appFont(size: 11)).foregroundColor(.gray).lineLimit(1)
                }
                Spacer()
                Text(mountain.difficulty.rawValue.uppercased())
                    .font(DesignSystem.Typography.appFont(size: 8, weight: .black))
...
struct XPDetailView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 30) {
                HStack {
                    Text("Performance Stats").font(DesignSystem.Typography.appFont(style: .title2)).fontWeight(.bold)
                    Spacer()
                    Button(action: { dismiss() }) { Image(systemName: "xmark.circle.fill").font(DesignSystem.Typography.appFont(size: 24)).foregroundColor(.gray) }
                }
                VStack(spacing: 8) {
                    Text("\(appState.currentXP) XP").font(DesignSystem.Typography.appFont(size: 48, weight: .black))
                    let region = appState.userRegion
                    Text(region.isEmpty || region == "Unknown" ? "Keep climbing to rank up!" : "Alpinist in \(region)")
                        .font(DesignSystem.Typography.appFont(style: .subheadline)).foregroundColor(.green)
                }
                .padding(.vertical, 20)
                HStack(spacing: 20) {
...
struct StatColumn: View {
    ...
    var body: some View {
        VStack(spacing: 4) {
            Text(title).font(DesignSystem.Typography.appFont(style: .caption)).foregroundColor(.gray).textCase(.uppercase)
            Text(value).font(DesignSystem.Typography.appFont(style: .title2)).fontWeight(.bold)
            Text(unit).font(DesignSystem.Typography.appFont(style: .caption2)).foregroundColor(.gray)
        }
        .frame(maxWidth: .infinity)
    }
}
...
struct BasecampMountainDetailSheet: View {
            ...
            VStack(spacing: 0) {
                ...
                ZStack(alignment: .topTrailing) {
                    if let urlStr = mountain.effectiveImageUrl, !urlStr.isEmpty, let url = URL(string: urlStr) {
                        ...
                        Image(systemName: "mountain.2.fill").font(DesignSystem.Typography.appFont(size: 40)).foregroundColor(Color.black.opacity(0.1))
                    }
                ...
                VStack(alignment: .leading, spacing: 16) {
                    HStack(alignment: .top) {
                        VStack(alignment: .leading, spacing: 4) {
                            Text(mountain.name).font(DesignSystem.Typography.appFont(size: 24, weight: .bold)).foregroundColor(.primary)
                            Text("\(mountain.region), \(mountain.country)").font(DesignSystem.Typography.appFont(size: 14)).foregroundColor(.secondary)
                        }
                        Spacer()
                        Text(mountain.difficulty.rawValue.uppercased())
    ...
    private func statItem(icon: String, value: String, label: String) -> some View {
        VStack(spacing: 6) {
            Image(systemName: icon).font(DesignSystem.Typography.appFont(size: 16)).foregroundColor(.secondary)
            Text(value).font(DesignSystem.Typography.appFont(size: 16, weight: .bold)).foregroundColor(.primary)
            Text(label).font(DesignSystem.Typography.appFont(size: 11)).foregroundColor(.secondary)
        }
        .frame(maxWidth: .infinity)
    }
}
...
struct LevelBadge: View {
    ...
    var body: some View {
        ZStack {
            ...
            Text("\(level)").font(DesignSystem.Typography.appFont(size: 14, weight: .black)).foregroundColor(gold)
        }
    }
}
...
struct WeeklyObjectiveCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                Image(systemName: icon).font(DesignSystem.Typography.appFont(size: 14, weight: .bold)).foregroundColor(color)
                Spacer()
                Text("\(Int(progress * 100))%").font(DesignSystem.Typography.appFont(size: 11, weight: .bold)).foregroundColor(progress >= 1 ? .green : color)
            }
            Text(title.uppercased()).font(DesignSystem.Typography.appFont(size: 10, weight: .black)).foregroundColor(.gray).tracking(1)
            HStack(alignment: .lastTextBaseline, spacing: 3) {
                Text("\(current)").font(DesignSystem.Typography.appFont(size: 24, weight: .black))
                Text("/ \(target) \(unit)").font(DesignSystem.Typography.appFont(size: 11, weight: .medium)).foregroundColor(.gray)
            }
            GeometryReader { geo in
                ZStack(alignment: .leading) {

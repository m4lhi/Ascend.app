        ...
        HStack(spacing: 0) {
            ...
            VStack(spacing: 2) {
                Text("+\(tour.xpGained)")
                    ...
                    .foregroundColor(accent)
                Text("XP")
                    .font(DesignSystem.Typography.appFont(size: 10, weight: .medium))
                ...
                if isLoading {
                    ...
                    VStack(spacing: 10) {
                        Image(systemName: "bubble.left.and.bubble.right")
                            .font(DesignSystem.Typography.appFont(size: 36)).foregroundColor(.gray.opacity(0.4))
                        Text("No comments yet")
                            .font(DesignSystem.Typography.appFont(style: .headline)).foregroundColor(.gray)
                        Text("Be the first to leave a comment!")
                            .font(DesignSystem.Typography.appFont(style: .caption)).foregroundColor(.gray.opacity(0.7))
                    }
                    Spacer()
                } else {
                    ScrollView {
...
struct StatBlock: View {
    ...
    var body: some View {
        HStack(spacing: 6) {
            if !icon.isEmpty { Image(systemName: icon).font(DesignSystem.Typography.appFont(size: 10)).foregroundColor(isXP ? .blue : .gray) }
            Text(value).font(DesignSystem.Typography.appFont(size: 12, weight: .bold)).foregroundColor(isXP ? DesignSystem.Colors.accent : .primary)
        }
